...
//
// The view is materialized right after the cast so that it dominates every
// consumer of the buffer and can be shared by them.
...
// CHECK-NEXT:       %[[VAL_7:.*]] = dataflow.get_local_unit %[[VAL_4]] {name = "lrfreg"} : index
// CHECK-NEXT:       %[[VAL_8:.*]] = dataflow.get_logical_memory_view %[[VAL_7]], %[[VAL_0]] {layout_map = #[[$ATTR_0]]} : index, index, memref<1x64xf16>
// CHECK-NEXT:       %[[VAL_9:.*]] = vectorchain.constant_bitstream {value = [0x0]} : vector<1xf16>
// CHECK-NEXT:       %[[VAL_10:.*]] = vectorchain.shuffle %[[VAL_9]] {indices = [0 : i32], repetition = 64 : i32} : vector<1xf16>, vector<64xf16>
// CHECK-NEXT:       agen.vector_store %[[VAL_10]], %[[VAL_6]]{{\[}}%[[VAL_1]], %[[VAL_1]]] {store_order = #[[$ATTR_1]], store_set = #[[$ATTR_2]]} : memref<1x64xf16>, vector<64xf16>
...
// CHECK-NEXT:       agen.vector_store %[[VAL_12]], %[[VAL_8]]{{\[}}%[[VAL_1]], %[[VAL_1]]] {store_order = #[[$ATTR_1]], store_set = #[[$ATTR_2]]} : memref<1x64xf16>, vector<64xf16>
// CHECK-NEXT:     }
// CHECK-NEXT:     return
// CHECK-NEXT:   }
