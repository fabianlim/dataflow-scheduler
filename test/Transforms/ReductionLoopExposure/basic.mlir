...
// Outer ktdf.private — memrefs and tokens are unchanged (no FIFOs at this level).
...
// MNILU stage is wrapped verbatim in scf.for — contents unchanged.
...
// Compute stage: outer batch scf.for, inner pipeline rebuilt.
...
// Inner ktdf.private — input FIFO shrunk from 16384 to 64 elements (R=256 divides it).
// CHECK:                 ktdf.pipeline {
...
// L1LU stage: scf.for over R; source_map updated to use loop IV at reduction dim; sizes patched.
...
// SFU stage: SFU_REG accumulator buffer + zeroing linalg.fill before the loop; the
// reduction scf.for carries no iter_args because the linalg.generic accumulates in
// place; write_to_fifo unconditional every iteration.
// CHECK:                   ktdf.stage depends_in(%[[VAL_6:.*]]#2) depends_out(%[[VAL_6]]#3) {
// CHECK-NEXT:                %[[ALLOC_2:.*]] = memref.alloc() : memref<1x64xf16, "SFU_REG">
// CHECK-NEXT:                %[[CST_0:.*]] = arith.constant 0.000000e+00 : f16
// CHECK-NEXT:                linalg.fill ins(%[[CST_0]] : f16) outs(%[[ALLOC_2]] : memref<1x64xf16, "SFU_REG">)
...
// L1SU stage: scf.for over R; data_transfer unconditional every iteration (last write wins).
// CHECK:                   ktdf.stage depends_in(%[[VAL_11:.*]]#3) depends_out(none) {
// CHECK-NEXT:                scf.for %[[VAL_12:.*]] = %[[CONSTANT_4]] to %[[CONSTANT_6]] step %[[CONSTANT_5]] {
...
// MNISU stage is unchanged.
// CHECK:               ktdf.stage depends_in(%[[VAL_13:.*]]#3) depends_out(none) {
