...
// absmax — absf(%in) maxnumf absf(%out) — must be seeded with 0.0, NOT the
// -inf a bare maxnumf takes: the accumulator passes through absf before the
// max, so a -inf seed arrives as +inf and swallows every answer.
...
module {
  ...
  module @local_schedule_neutral {
    ...
    // ── absmax (absf + maxnumf): neutral = 0.0 ────────────────────────────────
